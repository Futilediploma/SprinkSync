...
export async function exportMultiPiecePdf(project: any, pieces: any[]) {
    ...
    // Always generate image for PDF export
    {
      ...
      root.render(
        React.createElement(PipeSketch, {
          ...
          threadedFittings: pieces[i].threadedFittings || [],
  ...
  for (let index = 0; index < pieces.length; index++) {
    ...
    const separatorY = pieceStartY + (slotIndex + 1) * slotHeight - separatorHeaderGap;
    const slotBottom = slotIndex < piecesPerPage - 1 ? separatorY - slotGap : slotTop + slotHeight - slotGap - 8;
    let y = slotTop;
